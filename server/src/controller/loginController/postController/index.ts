...
const Unit = async (req:Request, res:Response) =>{
    try {
        ...
        // sign body to jwt;
        const token = jwt.sign(loginInfo, SECRET_KEY as string, {expiresIn: '600000'});
        const refreshToken = jwt.sign(loginInfo, SECRET_REFRESH_KEY as string, {expiresIn: '900000'});
